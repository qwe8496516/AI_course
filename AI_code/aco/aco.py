    ...
    for i in range(iterations):
        ...
        for ant_No in range(num_ants):
            ...
            while unvisited_cities:
                ...
                # next_city = max(probabilities, key=lambda x: x[1])[0]  # 選擇機率最大的城市
                # next_city = min(probabilities, key=lambda x: x[1])[0]  # 選擇機率最小的城市
        ...
        # 更新費洛蒙濃度
        for z in range(num_cities):
            for j in range(num_cities):
                pheromone_matrix[z][j] = pheromone_matrix[z][j] * (1 - evaporation_rate)

        for ant_path, ant_distance in zip(ant_paths, ant_distances):
            for z in range(len(ant_path) - 1):
                pheromone_matrix[ant_path[z]][ant_path[z+1]] += quality_factor / ant_distance
                pheromone_matrix[ant_path[z+1]][ant_path[z]] += quality_factor / ant_distance

        print(f"\nIteration {i+1}:")
        print("每一隻螞蟻的路徑:[ant1_path, ant2_path, ...]")
        print(ant_paths) # 印出每一隻螞蟻的路徑
